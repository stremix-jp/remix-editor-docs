...
「保存」はブラウザへのダウンロードです。サーバー上のドキュメントを更新する機能ではありません（[backend 連携](#backend-連携) を除く）。
...
保存した波形スニペットのライブラリをファイルとしてやり取りできます。

- **ファイル > 波形ライブラリをインポート...**（`.wavelib.json` / `.json`）
...
音声ファイルは **設定 > 一般 > 音声キャッシュ上限**（デフォルト 50MB）を超えると保存されません。詳しくは [再生操作](./07-playback.md) を参照してください。
...
トークンの権限とパラメータの条件を満たす場合のみ、画面上にドラッグで移動できるフローティングの「保存」ボタンが表示され、編集内容を backend へ保存できます（保存 / 保存中... / 保存済み / エラー）。
...
優先アクチュエーターが未設定のパターンがあると、エクスポートはブロックされます。パターン設定で優先アクチュエーターを設定してください。
